...
export class MatcherService {
  ...
  constructor(
    ...
    this.serverChanService = new ServerChanService(dbService);
    this.meowService = new MeowService(dbService);
  }

  private async dispatchNotifications(post: Post, subscription: KeywordSub, config: BaseConfig): Promise<boolean> {
    let sent = false;

    if (this.telegramService && config.stop_push !== 1 && config.bot_token && config.chat_id) {
      try {
        sent = await this.telegramService.pushPost(post, subscription) || sent;
      } catch (error) {
        logger.error(`Telegram 推送失败: ${post.title}`, error);
      }
    }

    if (this.serverChanService.isEnabled() && this.serverChanService.isConfigured()) {
      try {
        sent = await this.serverChanService.pushPost(post, subscription) || sent;
      } catch (error) {
        logger.error(`Server酱推送失败: ${post.title}`, error);
      }
    }

    if (this.meowService.isEnabled() && this.meowService.isConfigured()) {
      try {
        sent = await this.meowService.pushPost(post, subscription) || sent;
      } catch (error) {
        logger.error(`MeoW 推送失败: ${post.title}`, error);
      }
    }
  ...
  async processUnpushedPosts(): Promise<PushResult> {
    ...
    // 第三步：尝试推送已启用的通知通道
    const hasTelegramChannel = !!(this.telegramService && config.stop_push !== 1 && config.bot_token && config.chat_id);
    const hasServerChanChannel = this.serverChanService.isEnabled() && this.serverChanService.isConfigured();
    const hasMeowChannel = this.meowService.isEnabled() && this.meowService.isConfigured();

    if (!hasTelegramChannel && !hasServerChanChannel && !hasMeowChannel) {
      if (config.stop_push === 1) {
        logger.match('Telegram 推送已停止');
      }
    } else {
      let pushSuccess = 0;
      let pushFail = 0;
      ...
      for (const { post, subscription } of matchedPostsForPush) {
        try {
          const success = await this.dispatchNotifications(post, subscription, config);
      ...
      if (matchedPostsForPush.length > 0) {
        logger.info(`通知推送: ${pushSuccess} 成功, ${pushFail} 失败`);
      }
    }
    return result;
  }
  ...
  async manualPushPost(postId: number, subscriptionId: number): Promise<{
    ...
    try {
      ...
      if (!config) {
        return { success: false, message: '系统配置不存在' };
      }

      const pushSuccess = await this.dispatchNotifications(post, subscription, config);
