...
apiRoutes.post('/telegram/send-test', createValidationMiddleware(z.object({
    ...
    try {
        ...
        const result = await telegramService.testSendMessage(config.chat_id, testMessage);
...
apiRoutes.post('/notifications/serverchan/test', createValidationMiddleware(z.object({
    message: z.string().optional(),
})), async (c) => {
    try {
        const { message } = c.get('validatedData');
        const dbService = c.get('dbService');
        const serverChanService = new ServerChanService(dbService);

        if (!serverChanService.isConfigured()) {
            return c.json(createErrorResponse('Server酱 UID 或 SendKey 未配置'), 400);
        }

        const result = await serverChanService.testSend(message);
        if (result.success) {
            return c.json(createSuccessResponse(null, 'Server酱测试消息发送成功'));
        }

        return c.json(createErrorResponse(result.message), 400);
    } catch (error) {
        return c.json(createErrorResponse(`Server酱测试发送失败: ${error}`), 500);
    }
});

apiRoutes.post('/notifications/meow/test', createValidationMiddleware(z.object({
    message: z.string().optional(),
})), async (c) => {
    try {
        const { message } = c.get('validatedData');
        const dbService = c.get('dbService');
        const meowService = new MeowService(dbService);

        if (!meowService.isConfigured()) {
            return c.json(createErrorResponse('MeoW 昵称未配置'), 400);
        }

        const result = await meowService.testSend(message);
        if (result.success) {
            return c.json(createSuccessResponse(null, 'MeoW 测试消息发送成功'));
        }

        return c.json(createErrorResponse(result.message), 400);
    } catch (error) {
        return c.json(createErrorResponse(`MeoW 测试发送失败: ${error}`), 500);
    }
});
